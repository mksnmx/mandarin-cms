=== mandarin-pay ===
Contributors: mandarinltd
Donate link: https://mandarin.io/
Stable tag: 1.0
Requires at least: 4.7
Tested up to: 5.9
Requires PHP: 7.0
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

== Description ==

Mandarin Payment integration for WordPress (WooCommerce)

== Installation == 

[ENG]
Allows you to accept shopping cart payments through the Mandarin payment gateway.
1. Unpack the archive Wordpress_Woocommerce_Mandarin_new.zip to the root of the site
2. Administrator Panel - > Plugins - > MandarinPay - > Activate
3. Administrator Panel - > Plugins - > MandarinPay - > Set Up
4. Specify Merchant Id and Merchant Secret

[RUS]
Позволяет принимать оплату корзины через платежный шлюз Mandarin.
  1.  Распакуйте архив Wordpress_Woocommerce_Mandarin_new.zip в корень сайта
  2.  Панель администратора -> Плагины -> MandarinPay -> Activate
  3.  Панель администратора -> Плагины -> MandarinPay -> Настроить
  4.  Укажите Merchant Id и Merchant Secret
